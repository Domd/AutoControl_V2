...
void Posix_QextSerialPort::setParity(ParityType parity)
{
	...
	{
		if (parity == PAR_MARK || (parity == PAR_SPACE && Settings.DataBits
				== DATA_8))
		{
		}
		else
...
void Posix_QextSerialPort::setStopBits(StopBitsType stopBits)
{
	...
	{
		if ((Settings.DataBits == DATA_5 && stopBits == STOP_2) || stopBits
				== STOP_1_5)
		{
		}
		else
...
bool Posix_QextSerialPort::open(OpenMode mode)
{
	...
	if (!isOpen())
	{
		...
		if (Posix_File->open(QIODevice::ReadWrite | QIODevice::Unbuffered))
		{
			...
			Posix_CommConfig.c_lflag &= (~(ICANON | ECHO | ECHOE | ECHOK
					| ECHONL | ISIG));
			Posix_CommConfig.c_iflag &= (~(INPCK | IGNPAR | PARMRK | ISTRIP
					| ICRNL | IXANY));
...
qint64 Posix_QextSerialPort::bytesAvailable()
{
	...
	if (isOpen())
	{
		...
		int n = select(Posix_File->handle() + 1, &fileSet, NULL, &fileSet,
				&Posix_Timeout);
		if (!n)
		{
		...
		if (n == -1 || ioctl(Posix_File->handle(), FIONREAD, &bytesQueued)
				== -1)
		{
...
//set port name
void Posix_QextSerialPort::setPort(QString port)
{
	this->port = port;
}
