...
void CKongQi::on_pushButton_2_clicked()
{
    if(tablerow3<0)
    {
       return;
    ...
    QSqlQuery m_query(m_control->m_sql);
    m_query.prepare("delete FROM KongQiTable where truefunction=?");
...
void CKongQi::on_pushButton_3_clicked()
{
...
void CKongQi::SelectGuanXianAddr()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
        if(QString::compare(trUtf8("空气质量"),m_control->m_sensorstruct[i].name)==0)
        {
...
void CKongQi::SelectGuanXianControl()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
...
void CKongQi::on_tableWidget_cellClicked(int row, int column)
{
     tablerow1=row;
}

void CKongQi::on_tableWidget_2_cellClicked(int row, int column)
{
     tablerow2=row;
}

void CKongQi::on_tableWidget_3_cellClicked(int row, int column)
{
     tablerow3=row;
}
