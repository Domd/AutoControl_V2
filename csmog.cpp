...
void CSmog::on_pushButton_clicked()
{
    if (tablerow1<0|| tablerow2<0)
     ...
     m_query.prepare("insert into SmogTable(name,shortaddr,longaddr,dir,cname,cshortaddr,clongaddr,cdir,cfunction,truefunction) values(:name,:shortaddr,:longaddr,:dir,:cname,:cshortaddr,:clongaddr,:cdir,:cfunction,:truefunction)");

     m_query.bindValue(":name", trUtf8("烟感"));
...
void CSmog::on_pushButton_2_clicked()
{
    if(tablerow3<0)
    {
       return;
    ...
    QSqlQuery m_query(m_control->m_sql);
    m_query.prepare("delete FROM SmogTable where truefunction=?");
...
void CSmog::on_pushButton_3_clicked()
{
...
void CSmog::on_tableWidget_cellClicked(int row, int column)
{
     tablerow1=row;
}

void CSmog::on_tableWidget_2_cellClicked(int row, int column)
{
     tablerow2=row;
}

void CSmog::on_tableWidget_3_cellClicked(int row, int column)
{
    tablerow3=row;
}

void CSmog::readtable()//
{
    QSqlQuery m_query(m_control->m_sql);
...
void CSmog::SelectGuanXianAddr()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
        if(QString::compare(trUtf8("烟雾"),m_control->m_sensorstruct[i].name)==0)
        {
...
void CSmog::SelectGuanXianControl()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
