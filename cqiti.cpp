...
void CQiTi::on_pushButton_3_clicked()
{
...
void CQiTi::on_pushButton_2_clicked()
{
    if(tablerow3<0)
    {
       return;
    ...
    QSqlQuery m_query(m_control->m_sql);
    m_query.prepare("delete FROM QiTiTable where truefunction=?");
    m_query.bindValue(0,ui->tableWidget_3->item(ui->tableWidget_3->currentRow(),7)->text());
...
void CQiTi::on_pushButton_clicked()
{
    if (tablerow1<0|| tablerow2<0)
     ...
     m_query.prepare("insert into QiTiTable(name,shortaddr,longaddr,dir,cname,cshortaddr,clongaddr,cdir,cfunction,truefunction) values(:name,:shortaddr,:longaddr,:dir,:cname,:cshortaddr,:clongaddr,:cdir,:cfunction,:truefunction)");

     m_query.bindValue(":name", trUtf8("气体"));
...
void CQiTi::on_tableWidget_cellClicked(int row, int column)
{
     tablerow1=row;
}

void CQiTi::on_tableWidget_2_cellClicked(int row, int column)
{
     tablerow2=row;
}

void CQiTi::on_tableWidget_3_cellClicked(int row, int column)
{
     tablerow3=row;
}

void CQiTi::SelectGuanXianAddr()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
        if((QString::compare(trUtf8("可燃气体"),m_control->m_sensorstruct[i].name)==0))
        {
    ...
    {
        if((QString::compare(trUtf8("气体"),m_control->m_threefivestruct[i].name)==0))
        {
            ui->tableWidget->setRowCount(ui->tableWidget->rowCount()+1);
            ui->tableWidget->setItem(((ui->tableWidget)->rowCount()-1),0,new QTableWidgetItem(m_control->m_threefivestruct[i].longaddr));
            ui->tableWidget->setItem(((ui->tableWidget)->rowCount()-1),1,new QTableWidgetItem(m_control->m_threefivestruct[i].shortaddr));
            ui->tableWidget->setRowHeight(((ui->tableWidget)->rowCount()-1),50);
        }
...
void CQiTi::SelectGuanXianControl()
{
    for(int i=0;i<m_control->isensorlength;i++)
    {
